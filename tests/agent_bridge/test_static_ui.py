...
def test_every_persisted_event_kind_reduces_and_renders_full_safe_details() -> None:
    harness = r"""
      ...
      for (const expected of ["hash-only", "why_it_matters", "changed.py", "RuntimeError", "Review question"]) {
        if (!fullText.includes(expected)) process.exit(7);
      }
      if (created.filter((node) => node.tag === "details").length !== events.length) process.exit(8);
      ...
      const outcomeArticle = conversation.children[4];
